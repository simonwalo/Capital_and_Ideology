...
def simdim(models, title, keywords, key, *dims, rangelow=1850, rangehigh=2000, rangestep=10, stand=False, display=True):

    #check if not more than 3 dims
    if len(dims) > 3:
        raise ValueError(f"Too many dimensions: expected at most 3, got {len(dims)}")
    ...
    #show plot or return for further modification
